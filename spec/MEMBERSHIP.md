...
**v2 migration step — completed 26 May 2026.** Executed manually in the Stripe dashboard and Vercel env vars. The canonical config above is now live: **Arty £5/mo** unchanged; **CURE £50/mo** is a new product (`prod_UanhVQTezy6zri`, price `price_1Tbc6SAECn3p3G1JT04xkbsT`) with `metadata.club=cure` set on both the product and the price; the old £8 **"Both Clubs"** SKU is retired and not used. `STRIPE_PRICE_CURE` in Vercel was repointed at the new £50 price ID and production was redeployed. (Note: the new CURE tier was created as a *separate* product rather than a new price on the existing one, so the original £5 CURE product still exists in the dashboard.)
